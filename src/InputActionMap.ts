...
const inputControlsMap: { [key: string]: string } = {
    ...
    button2: "GO_BACK",
    dPadDown: "MOVE_DOWN",
    lStickDown: "MOVE_DOWN",
    dPadUp: "MOVE_UP",
    lStickUp: "MOVE_UP",
    dPadRight: "MOVE_RIGHT",
    lStickRight: "MOVE_RIGHT",
    dPadLeft: "MOVE_LEFT",
    lStickLeft: "MOVE_LEFT",
    rStickUp: "ROTATE_UP",
...
const gamePadControlMap: { [key: number]: any[] } = {
    /* deviceType */
    2: [
...
function normalizeJoystickInputs(stick: any): StickValues {
    const stickAngularSensitivity = 400;
    const stickMoveSensitivity = 40; // see above link
    const stickValues: StickValues = { x: 0, y: 0 };
    if (stick) {
...
function mapRotationInputToActions(stickInput: any, inputMap: any) {
...
function mapStickTranslationInputToActions(stickInput: any, inputMap: any) {
    // NOTE: I have no idea if this is a reasonable threshold value, test with 5.0.0-alpha23+
    if (stickInput.x >= 0.05) {
